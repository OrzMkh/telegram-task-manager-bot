...
async def check_sla_loop(app: Application, db_path: str = DB_PATH, sheets_sync: SheetsSyncManager = None):
    ...
    while True:
        try:
            now = get_now()
            # Fetch pending active tasks
            all_tasks = get_all_tasks(db_path=db_path, sheets_sync=sheets_sync)
            ...
            for task in all_tasks:
                ...
                status_clean = str(task.get("status", "")).strip().lower()
                # Skip finished or already expired/disputed tasks
                if status_clean in ("expired", "просрочена", "просрочено", "done", "completed", "выполнено", "выполнена", "удалена", "deleted", "disputed", "оспаривается"):
                    SENT_EXPIRED_ALERTS.add(task_id)
                    continue

                assignee = task.get("assignee", "Команда")
                task_text = task.get("task_text", "")
                sla_str = task.get("sla_deadline", "")
                ...
                if timedelta(seconds=0) < time_until_sla <= timedelta(hours=1):
                    if task_id not in SENT_REMINDERS and not task.get("reminder_sent"):
                        SENT_REMINDERS.add(task_id)
                        reminder_msg = (
                ...
                # 2. Check if SLA is expired (send ALERT strictly ONCE)
                elif now >= sla_dt:
                    if task_id not in SENT_EXPIRED_ALERTS:
                        SENT_EXPIRED_ALERTS.add(task_id)
                        alert_msg = (
                        ...
                        try:
                            ...
                            logger.info(f"SLA expired alert sent for task #{task_id}.")
                        except Exception as e:
