...
def test_task_filtering():
    ...
    # Check tasks for axi0603 without leading @
    axi_tasks = get_user_tasks("axi0603", status="Active", db_path=TEST_DB)
    assert len(axi_tasks) == 2, f"Expected 2 tasks for axi0603 (t2 + team), got {len(axi_tasks)}"
    axi_ids = [t["id"] for t in axi_tasks]
    assert t2["id"] in axi_ids
    assert t3["id"] in axi_ids
